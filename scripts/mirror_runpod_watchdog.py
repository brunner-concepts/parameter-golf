...
def build_summary(pod: dict[str, Any], remote: dict[str, Any] | None, mirror_state: dict[str, Any]) -> str:
    ...
    if remote:
        current = remote.get("current_state") or {}
        ...
        log_tail = (remote.get("active_log_tail") or "").strip()
        if log_tail:
            lines.extend(["", "## Active Log Tail", "", "```text", log_tail, "```"])
        next_action = (remote.get("next_action_txt") or "").strip()
        if next_action:
...
def event_key(pod: dict[str, Any], remote: dict[str, Any] | None) -> str:
...
def main() -> int:
    ...
    parser.add_argument("--exit-when-inactive", action="store_true", default=True)
    ...
    if mirror_state_path.exists():
        try:
            prior_state = json.loads(mirror_state_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError:
            prior_state = {}
    ...
    while True:
        ...
        mirror_state = {
            ...
            "event_key": event_key(pod, remote),
        }
        ...
        if mirror_state["event_key"] != previous_key:
            current = (remote or {}).get("current_state") or {}
            title = f"RunPod {args.pod_id}: {desired_status}"
            body = f"run={current.get('run_id')} status={current.get('status')} phase={current.get('phase_id')}"
